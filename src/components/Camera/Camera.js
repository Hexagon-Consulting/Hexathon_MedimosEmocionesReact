...
const Camera = ({ photoMode }) => {
  ...
  const getFaces = async () => {
    if (camera.current !== null) {
      ...
      setResults(faces);
      console.log(`faces: ${camera.current.video}`);
    }
  };
  ...
  return (
    <div className="camera">
      <p className="scroll_down">Mas resultados ↓</p>
      ...
      {photoMode ? (
        <>
          <div className="camera__button-container">
            {photos.length > 0 && <Button onClick={toggleGallery}>{showGallery ? 'Hide ' : 'Show '} Galeria</Button>}
